...
//
// Provides access to fury chain data.
//
...
func New(httpClient *fury.HTTPClient, wsClient *cosmos.WSClient, blockService *cosmos.BlockService, swaggerPath string) *API {
	r := mux.NewRouter()
	...
	r.HandleFunc("/health", func(w http.ResponseWriter, r *http.Request) {
		api.HandleResponse(w, http.StatusOK, map[string]string{"status": "up", "coinstack": "fury", "connections": strconv.Itoa(manager.ConnectionCount())})
	}).Methods("GET")
