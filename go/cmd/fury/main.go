...
func main() {
	...
	httpClient, err := fury.NewHTTPClient(cfg)
	if err != nil {
		logger.Panicf("failed to create new http client: %+v", err)
	}

	grpcClient, err := fury.NewGRPCClient(cfg)
	if err != nil {
		logger.Panicf("failed to create new grpc client: %+v", err)
	}

	blockService, err := fury.NewBlockService(httpClient)
	if err != nil {
		logger.Panicf("failed to create new block service: %+v", err)
	}

	wsClient, err := fury.NewWebsocketClient(cfg, blockService, errChan)
	if err != nil {
